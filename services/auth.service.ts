...
import Role, { RouteSchema } from "../models/role.model";

import { Strategy as LocalStrategy } from "passport-local";
const { getAccountIfValid, findById } = require("../services/account.service");
const RoleBinding = require("../services/roleBinding.service");
import * as logger from "./logger.service";

const emailAndPassStrategy = new LocalStrategy(
    {
        usernameField: "email",
    ...
    (email: string, password: string, done: any) => {
        ...
        getAccountIfValid(email, password).then(
            (account: Account) => {
                if (account) {
                    done(null, account);
                } else {
...
function deserializeUser(identifier: number, done: any) {
    findById(identifier).then(
        (user: Account) => {
            done(null, user);
        },
        (reason: Object) => {
 ...
 */
function serializeUser(user: Account, done: any) {
    done(null, user.identifier);
}

export { emailAndPassStrategy, deserializeUser, serializeUser };
...
export async function ensureAuthorized(req: Request, findByIdFns: any) {
    // if number of params doesn't match number of findById functions, fail
    //TODO - req.params is a ParamDictionary not string[]
    //@ts-ignore
    ...
    const roleBinding = await RoleBinding.getRoleBindingForAcct(
        //@ts-ignore TODO - User has no identifier field in express.User
        req.user?.identifier
    );
    if (!roleBinding) {
        return false;
    ...
    // get the route ids
    const twoDRoutes = roleBinding.roles.map((role: Role) => {
        return role.routes;
    });
    const routes: RouteSchema[] = [].concat(...twoDRoutes);
 ...
 */
function verifyParamsFunctions(
    params: string[],
    idFns: ((param: string) => object)[]
) {
 ...
 */
async function verifySelfCase(
    idFunction: (value: string) => object,
    param: string,
    userId: number
) {
    const object: any = await idFunction(param);
    if (!object) {
        return false;
    }
    ...
    if (object.accountId) {
        //TODO - Check if this code is correct, we changed to identifier from accountId.
        return object.identifier.toString() === userId;
    }
