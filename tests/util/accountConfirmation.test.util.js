...
const ConfirmationToken = Services.AccountConfirmation.generateToken(
    HackerConfirmation._id,
    HackerConfirmation.accountId
);
const FakeToken = Services.AccountConfirmation.generateToken(
    FakeHackerToken._id,
    FakeHackerToken.accountId
);
...
function store(attributes) {
    ...
    for (var i = 0; i < attributes.length; i++) {
        accountConfirmationDocs.push(
            new AccountConfirmationToken(attributes[i])
        );
        accountConfirmationIds.push(attributes[i]._id);
    }
    return AccountConfirmationToken.collection.insertMany(
        accountConfirmationDocs
    );
}
    ...
    try {
        ...
        if (e.code === 26) {
            logger.info(
                ...
                AccountConfirmationToken.collection.name
            );
        } else {
            throw e;
        }
